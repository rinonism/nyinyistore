...
import { getGameReviews, getAverageRating, type Review } from "@/lib/reviews";
...
export default function TopUpPage({ params }: TopUpPageProps) {
  ...
  // Reviews — fetch real from API, fallback to static
  const [reviews, setReviews] = useState<Review[]>(getGameReviews(game.slug));
  const [avgRating, setAvgRating] = useState(getAverageRating(getGameReviews(game.slug)));

  useEffect(() => {
    fetch(`/api/reviews?game_slug=${game.slug}&limit=10`)
      .then(res => res.json())
      .then(data => {
        if (data.reviews && data.reviews.length > 0) {
          const mapped: Review[] = data.reviews.map((r: { name?: string; rating: number; created_at: string; review: string; item_name?: string }) => ({
            name: r.name || "User",
            rating: r.rating,
            date: getRelativeDate(r.created_at),
            comment: r.review,
            item: r.item_name || "",
          }));
          setReviews(mapped);
          setAvgRating(getAverageRating(mapped));
        }
      })
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      // Fetch CSRF token before submitting order
      const csrfRes = await fetch("/api/csrf");
      const csrfData = await csrfRes.json();
      const csrfToken = csrfData.csrf_token;
      if (!csrfToken) {
        showToast("Gagal memuat security token. Refresh halaman.");
        setIsOrdering(false);
        return;
      }
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
            ...
            <div className="hidden lg:block w-[320px] flex-shrink-0">
              <div className="sticky top-4 space-y-4">
                {/* Rating Card */}
                <div className="rounded-xl border border-[#d4af37]/30 bg-[#1e1e1e] p-5">
                  <div className="text-center mb-3">
                    <p className="text-2xl mb-1">⭐</p>
                    <p className="text-lg font-bold text-[#d4af37]">{avgRating}/5</p>
                    <p className="text-[10px] text-[#777]">{reviews.length} ulasan</p>
                  </div>
                  <div className="space-y-2 max-h-[300px] overflow-y-auto">
                    {reviews.slice(0, 3).map((review, idx) => (
                      <div key={idx} className="border-t border-[#2a2a2a] pt-2">
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="text-[11px] font-medium text-white">{review.name}</span>
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <span key={i} className={`text-[9px] ${i < review.rating ? "text-[#d4af37]" : "text-[#555]"}`}>★</span>
                            ))}
                          </div>
                        </div>
                        <p className="text-[10px] text-[#b0b0b0] leading-relaxed">{review.comment}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <section className="rounded-xl border border-[#2a2a2a] bg-[#1e1e1e] overflow-hidden">
              <div className="flex items-center gap-2 border-b border-[#2a2a2a] bg-[#252525] px-3 sm:px-4 py-2.5 sm:py-3">
                ...
                <span className="ml-auto text-[10px] text-[#d4af37] font-medium">{avgRating}/5 ({reviews.length} ulasan)</span>
              </div>
                  ...
                  <div key={idx} className="border-b border-[#2a2a2a] pb-3 last:border-0 last:pb-0">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-[#777]">{review.date}</span>
                      </div>
                      <div className="flex gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <span key={i} className={`text-[10px] ${i < review.rating ? "text-[#d4af37]" : "text-[#555]"}`}>★</span>
                        ))}
                      </div>
                    </div>
                    <p className="text-[11px] text-[#b0b0b0] leading-relaxed">{review.comment}</p>
                    <p className="text-[10px] text-[#555] mt-1">Item: {review.item}</p>
                  </div>
                ))}
              </div>
            </section>
