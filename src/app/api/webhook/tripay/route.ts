...
export async function POST(request: NextRequest) {
  try {
    ...
    // Amount verification: ensure paid amount matches order price
    if (status === "PAID" && total_amount && order.price_idr) {
      if (total_amount < order.price_idr) {
        console.error(`Webhook: Amount mismatch! Paid=${total_amount}, Expected=${order.price_idr}, Order=${merchant_ref}`);
        // Notify admin but don't fulfill
        try {
          await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
            ...
            body: JSON.stringify({
              ...
              text: `🚨 *ALERT: Amount Mismatch!*\n\nOrder: \`${merchant_ref}\`\nPaid: Rp ${total_amount.toLocaleString("id-ID")}\nExpected: Rp ${order.price_idr.toLocaleString("id-ID")}\n\n⛔ Auto-fulfill BLOCKED. Investigate!`,
              parse_mode: "Markdown",
            }),
          });
        } catch (e) {}
    ...
    if (status === "PAID") {
      ...
      // Auto-fulfill: call via proxy (VPS IP whitelisted by Digiflazz)
      try {
        const fulfillResult = await autoFulfillOrder(merchant_ref);
        if (!fulfillResult.success) {
          console.error("Auto-fulfill failed:", fulfillResult.error);
        }
      } catch (e) {
