...
const DIGIFLAZZ_PROXY_SECRET = process.env.TRIPAY_PROXY_SECRET || "";
const DIGIFLAZZ_PROXY_URL = process.env.TRIPAY_PROXY_URL || "http://43.153.204.244:3847";
...
async function handleCreate(body: any) {
  ...
  try {
    await fetch(`${DIGIFLAZZ_PROXY_URL}/save-order`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Proxy-Secret": DIGIFLAZZ_PROXY_SECRET },
...
async function handleGet(body: any) {
  ...
  if (!order) {
    try {
      const res = await fetch(`${DIGIFLAZZ_PROXY_URL}/get-order`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Proxy-Secret": DIGIFLAZZ_PROXY_SECRET },
...
async function handleCheck(body: any) {
  ...
  try {
    const res = await fetch(`${DIGIFLAZZ_PROXY_URL}/check-payment`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Proxy-Secret": DIGIFLAZZ_PROXY_SECRET },
      body: JSON.stringify({ order_id }),
    });
